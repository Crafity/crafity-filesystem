            ...
            if (c.isDirectory()) {
              fs.readdir(f, function (err, nfiles) {
                ...
                nfiles.forEach(function (b) {
                  var childFile = path.join(f, b);
                  if (!files[childFile]) {
                    fs.stat(childFile, function (err, stat) {
                      if (err) { return; }
                      if (options.ignoreDotFiles && path.basename(childFile)[0] === '.') {return;}
                      if (options.filter && options.filter(childFile, stat)) {return;}
                      files[childFile] = stat;
                      fileWatcher(childFile);
                      callback(childFile, stat, null);
                    });
                  }
                });
              });
