...
function Filesystem() {
	...
	this.getAllFiles = function (path, pattern, deep, callback) {
		...
		fs.readdir(path, function (err, objects) {
			if (err) { return callback(err); }

			objects.forEach(function (object) {
				fs.stat(self.combine(path, object), function (err, stat) {
					if (deep && stat.isDirectory()) {
						return process.nextTick(function () {
							return process.nextTick(function () {
								callback(null, new Directory(path, object));
								return self.getAllFiles(self.combine(path, object), pattern, deep, callback);
							});
						});
					} else if (stat.isFile()) {
